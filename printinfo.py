...
def readFile(f):
        ...
        for i in range(count):
            ...
            if trkInfoTable[i].offset not in [0xffffffff, 0]:
                ...
                if channelIndexByteTable_ref.offset not in [0xffffffff, 0]:
                    print("Channel Index Byte Table offset: " + hex(channelIndexByteTable_ref.offset + pos-trkInfo[i].size))
    ...
    for i in range(count):
        ...
        if channelInfoTable[i].offset not in [0xffffffff, 0]:
            print('')
            print("Channel " + str(i) + " Info Entry ADPCM Info Reference offset: " + hex(channelInfoTable[i].offset + pos-4))
