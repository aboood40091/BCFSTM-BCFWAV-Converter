...
def convFile(f, dest, dest_bom):
    ...
    if magic in supp_STM and dest in supp_STM:
        outputBuffer = STMtoSTM(f, dest, dest_bom)
    elif magic in supp_WAV and dest in supp_WAV:
        outputBuffer = WAVtoWAV(f, dest, dest_bom)
    elif magic in supp_STM and dest in supp_WAV:
        print('')
        print("BFSTM/BCSTM/BFSTP to BFWAV/BCWAV is not implemented!")
        print('')
        ...
        print('')
        print("BFWAV/BCWAV to BFSTM/BCSTM/BFSTP is not implemented!")
        print('')
        print("Exiting in 5 seconds...")
        time.sleep(5)
        sys.exit(1)
...
def main():
    ...
    if "-bom" in sys.argv:
        bom = int(sys.argv[sys.argv.index("-bom") + 1], 10)
        if bom == 0:
            dest_bom = '>'
        elif bom == 1:
            dest_bom = '<'
